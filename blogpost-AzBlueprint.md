...
There many techniques to control the deployment of an environment. Techniques range from the low tech spreadsheet with a checklist of tasks to the modern infrastructure-as-code technologies like Terraform or Ansible. In my view, checklists are to slow and error-prone and infrastructure-as-code technologies need too much ramp-up time to be effective for the small Azure environments our customers need. Microsoft might have hit the sweet spot between simplicity and power with the Azure Blueprint service.
...
2. **Publish your Blueprint** - After your Blueprint is ready to go, publish it so it by giving it a version number and description. It is now ready to be assigned.
...
At the time of assignment you can choose to use Lock Assignments to protect your design. The Lock Assignment allows you protect the Artifacts you've defined as read-only or to prevent them from being deleted, even by subscription owners. 
...
1. Adding or removing resource locking or changing the type
